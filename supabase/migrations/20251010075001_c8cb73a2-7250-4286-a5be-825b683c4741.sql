-- Remove weekly limit functionality, keep only monthly limits

-- 1. Update can_user_register_for_course_enhanced to remove weekly logic
  ...
  IF booking_rules IS NOT NULL THEN
    ...
    ELSIF booking_rules->>'type' = 'limited' THEN
      ...
      BEGIN
        ...
        RAISE NOTICE '[Registration Check] Limited membership. Count: %, Target period: % to %', 
          limit_count, target_period_start, target_period_end;
        ...
        -- Count registrations in TARGET MONTH (based on course_date)
        SELECT COUNT(*)
        INTO used_in_period
        FROM public.course_registrations cr
        ...
        IF used_in_period >= limit_count THEN
          RAISE NOTICE '[Registration Check] BLOCKED: Monthly limit reached (% >= %)', used_in_period, limit_count;
          RETURN '{"canRegister": false, "canWaitlist": false}'::jsonb;
        END IF;
...
-- 2. Convert existing "Limited 3x/Week" memberships to "Limited 12x/Month"
UPDATE public.membership_plans_v2
SET 
  name = REPLACE(name, '3x/Week', '12x/Month'),
  booking_rules = jsonb_set(
    jsonb_set(
      booking_rules,
      '{limit,period}',
      '"month"'
...
WHERE booking_rules->>'type' = 'limited'
  AND booking_rules->'limit'->>'period' = 'week';