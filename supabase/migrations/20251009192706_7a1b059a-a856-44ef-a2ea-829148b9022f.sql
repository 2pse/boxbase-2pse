-- Fix can_user_register_for_course_enhanced function for Limited memberships
-- Remove the incorrect remaining_credits check for Limited memberships
...
BEGIN
  ...
  IF booking_rules IS NOT NULL THEN
    ...
    ELSIF booking_rules->>'type' = 'limited' THEN
      -- Limited memberships: Only check period limits, NOT remaining_credits
      DECLARE
      ...
      BEGIN
        ...
        IF limit_period = 'week' THEN
          current_period_start := date_trunc('week', CURRENT_DATE::timestamp)::date + 1;
          is_future_period := target_period_start >= current_period_start + 7;
        ELSE
          current_period_start := date_trunc('month', CURRENT_DATE::timestamp)::date;
          is_future_period := target_period_start >= (current_period_start + interval '1 month')::date;
        END IF;
        ...
        IF is_future_period THEN
          ...
          END IF;
        ELSE
          -- Current or same period: Only check period limits (NOT credits!)
...
-- Cleanup: Remove incorrect remaining_credits from Limited memberships
...
AND membership_data ? 'remaining_credits';