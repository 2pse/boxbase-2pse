-- Fix ambiguous column reference in handle_course_registration_credits function
CREATE OR REPLACE FUNCTION public.handle_course_registration_credits(p_user_id uuid, p_course_id uuid, p_action text)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
DECLARE
  booking_rules jsonb;
  user_membership_data jsonb;  -- Renamed from membership_data to avoid ambiguity
...
BEGIN
  ...
  SELECT 
    ...
    mp2.booking_rules,
    um2.membership_data
  INTO membership_id, booking_rules, user_membership_data  -- Using renamed variable
  ...
  -- Get current credits from renamed variable
  current_credits := COALESCE((user_membership_data->>'remaining_credits')::integer, 0);
