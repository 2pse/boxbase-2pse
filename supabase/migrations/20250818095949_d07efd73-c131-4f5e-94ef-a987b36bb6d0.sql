-- Phase 2: Kern-Tabellen erweitern und Challenge-System implementieren
...
-- 3. Training Sessions korrigieren - date zu session_date umbenennen falls nötig
DO $$
BEGIN
    IF EXISTS (
        ...
        ALTER TABLE public.training_sessions RENAME COLUMN date TO session_date;
    END IF;
END $$;

-- Erweitere training_sessions mit zusätzlichen Feldern
...
-- 9. Wichtige Constraints hinzufügen
-- Unique Constraint für training_sessions erweitern
ALTER TABLE public.training_sessions DROP CONSTRAINT IF EXISTS training_sessions_user_id_date_workout_type_key;
ALTER TABLE public.training_sessions ADD CONSTRAINT training_sessions_user_id_session_date_workout_type_unique 
UNIQUE(user_id, session_date, workout_type);

-- 10. RLS für alle neuen Tabellen aktivieren
