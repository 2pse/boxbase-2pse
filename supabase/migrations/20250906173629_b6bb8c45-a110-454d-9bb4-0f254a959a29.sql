-- Update the can_user_register_for_course_v2 function to properly check credits for limited memberships
...
BEGIN
  ...
  IF booking_rules IS NOT NULL THEN
    ...
    ELSIF booking_rules->>'type' = 'limited' THEN
      -- Limited memberships also need credits AND must check period limits
      user_credits := COALESCE((membership_data->>'remaining_credits')::integer, 0);
      ...
      -- Then check period limits for limited memberships
      DECLARE
